...
exports.arrayOfGizmoURLs = function (arr) {
  arr = Array.isArray(arr) ? arr : [arr]
  return arr.map(v => {
    if (!v) return false
    if (typeof v === 'string') {
      return exports.recordUrl(v)
    }
    if (v.gizmoOriginURL && typeof v.gizmoOriginURL === 'string') {
      return exports.recordUrl(v.gizmoOriginURL)
    }
  }).filter(Boolean)
}
